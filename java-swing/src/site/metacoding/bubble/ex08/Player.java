...
public class Player extends JLabel { // JFrame위에 올려놓았다.
	...
	private int direction; // 0은 왼쪽 방향, 1은 오른쪽 방향, -1은 멈춤
	...
	public Player(BubbleFrame context) { // 생성자 주입
	...
	public void attack() {
		...
		// 화면에 붙여야 함, 버블 생성은 플레이어의 책임
		context.add(bubble);
		// 수평발사(플레이어의 방향)
		
		if(direction == 0) {
			System.out.println("왼쪽 발사");
			bubble.left();
		} else if(direction == 1) {
			System.out.println("오른쪽 발사");
			bubble.right();
		}
	}
